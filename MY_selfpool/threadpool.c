/*************************************************************************
    ...
    > Author: ZY
    > Created Time: 2017年06月20日 星期二 15时49分18秒
 ************************************************************************/
...
threadpool_t *threadpool_create(int thread_count, int queue_size, int flags)
{
    ...
    /* 创建指定数量的线程开始运行 */
    for(i = 0; i < thread_count; i++) {//这里创建线程函数要注意 如果任务队列无任务会阻塞在线程函数那里
                   ...
                   void *argument, int flags)
{
    ...
    do {
         ...
         * ！！！！！carefully如果由因为任务队列空阻塞的线程，此时会有一个被唤醒
         * 如果没有则什么都不做
         */
...
static void *threadpool_thread(void *threadpool)
{
    ...
    for(;;) 
	{
			...
			while((pool->count == 0) && (!pool->shutdown)) {
				//理解下面这句话很总要very important！！！！！！
				//刚开始启动程序的时候 任务队列肯定为空  
				//此时线程函数是阻塞的  需要等待任务队列被添加任务后 唤醒
			...
			/* Grab our task */
			/* 取得任务队列的第一个任务 */
			//即 用得到的线程 开始取  任务队列的任务  开始 干活了   每个线程函数依次获取任务
